...
class RSSIngestor extends BaseIngestor {
    ...
    async process(source, sourceHash, feedUrl, clientBucketName) {
        const result = await this.fetch(feedUrl, source.etag, source.lastModified);
        if (result.modified) {
            console.log(`  -> Processing ${sourceHash}...`);
            const sanitizedXml = result.xml.replace(/&(?!amp;|lt;|gt;|quot;|apos;|#[0-9]+;|#x[0-9a-fA-F]+;)/g, '&amp;');
            ...
            if (clientBucketName) {
                uploadPromises.push(this.uploadToR2(`feeds/${sourceHash}.json`, manifest, 'application/json', clientBucketName).catch(e => {
                    console.error(`  !! Client bucket upload failed for ${sourceHash}: ${e.message}`);
                }));
            }
            ...
            if (clientBucketName) {
                this.uploadToR2(`feeds/${sourceHash}.json`, [], 'application/json', clientBucketName).catch(e => {
                    console.error(`  !! Client bucket upload failed for ${sourceHash}: ${e.message}`);
                });
            }
