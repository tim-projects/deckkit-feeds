...
class RSSIngestor extends BaseIngestor {
    ...
    async process(source, sourceHash, feedUrl) {
        ...
        if (result.modified) {
            ...
            if (this.clientBucketName) {
                uploadPromises.push(this.uploadToBucket(this.clientBucketName, `feeds/${sourceHash}.json`, manifest));
            }
            ...
            if (this.clientBucketName) {
                await this.uploadToBucket(this.clientBucketName, `feeds/${sourceHash}.json`, []);
            }
            source.brokenSince = source.brokenSince || new Date().toISOString();
