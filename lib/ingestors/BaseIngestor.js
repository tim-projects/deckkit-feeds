...
class BaseIngestor {
    ...
    sanitizeXml(xml) {
        const tags = [];
        const withPlaceholders = xml.replace(/<[^>]*>/g, (tag) => {
            tags.push(tag);
            return `\x00${tags.length - 1}\x00`;
        });
        const escaped = withPlaceholders.replace(/&(?!amp;|lt;|gt;|quot;|apos;|#[0-9]+;|#x[0-9a-fA-F]+;)/g, '&amp;');
        return escaped.replace(/\x00(\d+)\x00/g, (_, i) => tags[i]);
    }
